...
# 6. Calculates NDVI index from clipped images
# 7. SendS to PostGIS DB the NDVI and other images *NOT WORKS YET*
...
# 2. Downloads Sentinel2 images of AOI using sentinelsat API from SCIHUB
zip_path_list = download_sentinel2_images(dic_AOI)
...
for sid in zip_path_list:
    ...
    bands_names_list, bands_folder_path, bands_path_list = unzip_transform(
                                                           workspace,
                                                           zip_path_list[sid])
    # 4. Creates shp from AOI and gets envelope
    ...
    # Creates folder to store clipped images
