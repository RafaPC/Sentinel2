...
# Creamos diccionario vacio donde almacenar clave(nombre shp) y valor (geometry de cada feature)
# Crearemos un diccionario de listas
geometryDict = {}
...
for shp in range(datasource.GetLayerCount()):
	...
	# Creamos lista vacia donde almacenaremos las geometrias de los features contenidos en cada shp
	# Al estar dentro de la iteracion, la lista se sobreescribira para cada shp
	feature_geom_list = []
	...
	for feature in layer:
		...
		#Anadimos la geometria en lat/long a la lista correspondiente a su shp
		feature_geom_list.append(feature)

	# print (feature_geom_list)

	# Tras acabar las iteraciones sobre los features del shp, anadimos todas sus geometrias al diccionario, el comando update
	# No anadira geometrias duplicadas para una misma clave, es decir, si un mismo shp (clave) tiene dos parcelas con geometria
	# identicas (valor) no la anadira, evitando duplicados

	geometryDict.update({layer.GetName(): feature_geom_list})
for featureList in geometryDict:
	print ("Lista de geometrias de los features contenidos en ", featureList)
	for feature in geometryDict.get(featureList):
		print (feature.GetGeometryRef())

#	# Actualmente, lo de abajo, ya no hace falta pues las geometrias se cogen y transforman adecuadamente
#	# Lo conservaremos por posibles pruebas o cambios futuros
...
for clave in geometryDict:
	for parcela in geometryDict.get(clave):
		geom = parcela.GetGeometryRef()
...
# hacemos order_by por atributo (indicando +(default) o - delante para ordenar ascending o descending)
		...
		# Capturamos el error de conexion al servidor
			...
			if tries < 3 and responseOK is False:
					time.sleep(3.0)
		# Seleccionamos el primer producto por su clave, si su id ya ha sido descargada, continuamos con la iteracion general
		if responseOK is True:	
			print (len(products_list))
			id = list(products_list.keys())[0]
			if id in idList:
				print ("para la %s la id %s esta repetida") %(parcela, id)
				continue
		# Si el id de descarga no existia, append a la lista y descargamos
			...
			# print (respuesta)
			print ('Descarga Completada del archivo con id %s para la parcela \n%s \ndel shp %s') %(id, geom, clave)
		else:
			print ("La parcela %s del shp %s no se ha podido descargar") %(feature, clave)
			continue
